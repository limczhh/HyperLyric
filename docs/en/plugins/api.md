...
`PluginCacheEntry` may carry optional `details: List<PluginCacheDetail>` (label/value pairs; labels must already be localized by the plugin, and values are display-only text). For entries with `details`, the host provides a dedicated detail page that renders all detail lines (multi-value fields wrap onto new lines) and hosts single-entry deletion (a delete icon in the top bar plus a confirmation dialog). Entries without `details` show a placeholder. "Clear all" remains in the list-page top bar. Detail lines are bounded by host budgets: 12 lines per entry, 32-char labels, and 120-char values.
