...
- `cache`：保存可丢弃的结果，支持字符串和字节读写、存在性检查、删除和清空。插件负责 key、序列化、版本和失效策略，宿主负责按插件 ID 隔离和异常保护。Manifest 未声明 `cacheScopes` 时，宿主提供无操作缓存；插件未写入有效正文前不会创建缓存文件。
- `logger`：使用 `debug`、`info`、`warn` 和 `error`，不要直接写 Android 日志或自定义日志文件。

缓存正文决定命中，插件自维护的索引只用于列表展示；索引缺失不应阻止按 key 读取正文。缓存损坏、读取失败或超限时，应忽略或删除当前条目，回退到网络或无结果路径，不要清空原始歌词。缓存不要保存整份可能过期的 `PluginSong`；应保存可以基于当前歌曲重新应用的结果。
