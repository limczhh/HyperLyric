 ...
 *
 * 命中时完整替换歌词（name/artist 等保留主歌词源值）；
 * 功能在 34s 总预算内同步完成（宿主硬超时 40s），每步网络前检查预算与线程中断。
 */
internal class AmllTtmlFeature(
    ...
    private val preferenceListener =
        SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            if (key == null || key in AmllTtmlConfig.PREFERENCE_KEYS) {
                onConfigChanged()
            }
        }
    ...
    /**
     * 解析 TTML 并构造歌词：仅替换 lyrics，name/artist 等保留主歌词源值
...
internal object LyricsValidator {
    ...
    fun isValidLyrics(lines: List<RichLyricLine>): Boolean {
    ...
    private fun hasValidWords(line: RichLyricLine, words: List<LyricWord>?): Boolean {
